...
contract('Get Payload Detail Test', (accounts) => {
  ...
  beforeEach('deploy contract and store a valid payload', async () => {
    ...
    await RC.storeAuthNPayload(payloadHash, gatewayAddress, ISPAddress, { from: ownerAddress });
  });
  ...
  it('OBSERVER can query FAKE or VALID PAYLOAD', async () => {
    ...
    let payload = await RC.getPayloadDetail(fakeHash, { from: observerAddress });
    ...
    payload = await RC.getPayloadDetail(payloadHash, { from: observerAddress });
