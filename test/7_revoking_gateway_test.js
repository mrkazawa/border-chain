...
contract('Revoking Gateway Test', (accounts) => {
  ...
  beforeEach('deploy contract and store a valid payload', async () => {
    ...
    await RC.storeAuthNPayload(gatewayPayloadHash, gatewayAddress, ISPAddress, { from: ownerAddress });
  });

  it('DOMAIN OWNER can NOT revoke the GATEWAY due to invalid PAYLOAD (not exist)', async () => {
    await RC.verifyAuthNGateway(gatewayPayloadHash, { from: ISPAddress });
    const fakeHash = '0x017dfd85d4f6cb4dcd715a88101f7b1f06cd1e009b2327a0809d01eb9c91f000';
    await truffleAssert.reverts(
      RC.deleteTrustedGateway(fakeHash, gatewayAddress, { from: ownerAddress }), 'payload must exist'
    );
  });
  ...
  it('OBSERVER can NOT revoke the GATEWAY due to invalid SOURCE', async () => {
    ...
    await truffleAssert.reverts(
      RC.deleteTrustedGateway(gatewayPayloadHash, gatewayAddress, { from: observerAddress }), 'only for original source'
    );
  });
  ...
  it('DOMAIN OWNER can NOT revoke the GATEWAY due to has not been verified yt', async () => {
    ...
    await truffleAssert.reverts(
      RC.deleteTrustedGateway(gatewayPayloadHash, gatewayAddress, { from: ownerAddress }), 'gateway must be trusted first'
    );
  });

  it('DOMAIN OWNER can revoke the GATEWAY properly', async () => {
    await RC.verifyAuthNGateway(gatewayPayloadHash, { from: ISPAddress });
    await RC.storeAuthNPayload(devicePayloadHash, deviceUUID, vendorAddress, { from: gatewayAddress });
    await RC.verifyAuthNDevice(devicePayloadHash, { from: vendorAddress });

    let status = await RC.isTrustedGateway(gatewayAddress, { from: observerAddress });
    assert.equal(status, true, "gateway address has been put into the trusted list");
    status = await RC.isTrustedDevice(deviceUUID, { from: observerAddress });
    assert.equal(status, true, "device address has been put into the trusted list");

    let tx = await RC.deleteTrustedGateway(gatewayPayloadHash, gatewayAddress, { from: ownerAddress });
    truffleAssert.eventEmitted(tx, 'GatewayRevoked', { sender: ownerAddress, gateway: gatewayAddress });
    ...
    status = await RC.isTrustedGateway(gatewayAddress, { from: observerAddress });
